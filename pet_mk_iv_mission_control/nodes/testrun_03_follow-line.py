...
# 1) Follow a line on the floor. By swaying back and forth...
# 2a) Rule "a": ALL sensors detecting line -> STOP
# 2b) Rule "b": ANY distance sensors to close to obstacle => STOP
...
class MissionNode(object):
  ...
  #  kLinearSpeed   = 0.0 # m/sec
  #  kRotationSpeed = 0.0 # rad/sec
    ...
    def check_for_stop(self, msg):
        # Stop criteria #1: ALL sensors detecting line -> STOP!
        if  (    self.line_detection_left   == LineDetection.DARK
            ...
            rospy.logwarn("STOP! <= Stop line detected!")
            self.row1_pub.publish("STOP")
            self.stop_handler()

        # Stop criteria #2: ANY distance sensors to close to obstacle -> STOP!
        elif (
                 0 < self.range_sensor_front_left  < MissionNode.kSideDistance
              or 0 < self.range_sensor_middle_left < MissionNode.kForwardDistance 
              or 0 < self.range_sensor_front_right < MissionNode.kSideDistance
             ):
            rospy.logwarn("STOP! <= Obstacles detected")
            self.row1_pub.publish("STOP")
    ...
    def follow_line(self, msg):
        if (self.is_stopped):
            rospy.loginfo("FollowLine: IsStoped")
            ...
            rospy.loginfo("FollowLine: L,D,L = On Track!")
            ...
            rospy.loginfo("FollowLine: L,L,D = Turn Right!")
        ...
        rospy.logdebug("FollowLine: -> Done")
        return
    ...
    def start_handler(self):
        ...
        self.beacon_mode_pub.publish(self.beacon_msg)
        rospy.loginfo("FollowLine > start_handler: Done")

    def stop_handler(self):
        self.is_stopped = True
        self.beacon_msg.mode = LightBeacon.ROTATING_SLOW
        self.beacon_mode_pub.publish(self.beacon_msg)
        rospy.loginfo("FollowLine > stop_handler = Done")

    def abort_handler(self):
        self.is_stopped = True
        self.beacon_msg.mode = LightBeacon.OFF
        self.beacon_mode_pub.publish(self.beacon_msg)
        rospy.loginfo("FollowLine > IR-remote STOP = Abort mission script")
